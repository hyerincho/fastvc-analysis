...
def compare_cost_mz_oz():
    oz_files = ["093025_oz_a0.9_T+_momcons/out-38004434.txt", "050625_a0.9_rB2e5_oz_test/out-14399782.txt"]
    oz_times = []
    for fname in oz_files:
        first_time = 1e9; last_time = 0.
        hlines = head(fname, 10000)
        tlines = tail(fname, 10000)
        dump_fname = glob.glob(fname.split('/')[0] + "*out0.00000.phdf")[0]
        dump = pyharm.load_dump(dump_fname,ghost_zones=False)
        r_sonic = dump["rs"]
        mdot = dump["mdot"]
...
def _main():
    print_walltime_summary(["052825_a0.9_rB2e5_bondi_eks_largerout"])
    if 0: # count average cycles for each zone for tchar runs
