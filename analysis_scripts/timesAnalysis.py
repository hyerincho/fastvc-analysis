...
def calc_vchar(dump):
    r = dump["r1d"]
    cs2 = 27. * dump["gam"] / (80. * dump["rs"]**2)
    vchar = np.sqrt(1./r + cs2)
    return vchar
...
def max_velocities(dump, ax_passed=None):
    ...
    # formatting
    ax.set_xlabel(pyharm.pretty('r')); ax.set_ylabel(pyharm.pretty('v'))
    ax.set_ylim([1e-7,3])
    ax.legend()
...
def betaGamma(dump, ax_passed=None, norm_vchar=False):
    ...
    bg = np.sqrt(np.power(dump["Gamma"],2.)-1.)
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    if norm_vchar: ax.set_ylabel(r"$(\beta\gamma)_{\rm max} / v_{\rm char}$")
    else: ax.set_ylabel(r"$(\beta\gamma)_{\rm max}$")
    ax.legend()
...
def Gamma(dump, ax_passed=None):
  ...
  ax.set_xlabel(pyharm.pretty('r')); ax.set_ylabel(r"$\gamma_{\rm max}$")   
  if ax_passed is None:
